...
#define PROGRAM_NAME "ircddbmhd3"
...
static void flush_mheard_data_module(int i)
{
  if (stream_info[i].mheard_info_timer > 0)
  {
    ...
    if (r != packet_len)
    {
      fprintf(stderr, "send didn't work, retval=%d\n", r);
      fflush(stderr);
    }
...
static void usage(const char * a)
{
  fprintf (stderr, PROGRAM_NAME " version '%s'\n"
	"Usage: %s <conf-file> <pcap-file>    # test-mode, read packets from pcap file\n"
	"Usage: %s <conf-file>                # read packets from eth interface\n",
...
int main(int argc, char *argv[])
{
  ...
  if(!config_lookup_string(&cfg, "CAPTURE_INTERFACE", &capture_interface))
  {
    fprintf(stderr, "no setting CAPTURE_INTERFACE in configuration file. Abort.\n");
    return 2;
  }

  const char * zr_addr;
 
  if(!config_lookup_string(&cfg, "ZR_ADDR", &zr_addr))
  {
    fprintf(stderr, "no setting ZR_ADDR in configuration file. Abort.\n");
    return 2;
  }

  int udp_port;

  if(!config_lookup_int(&cfg, "MHEARD_UDP_PORT", &udp_port))
  {
    fprintf(stderr, "no setting MHEARD_UDP_PORT in configuration file. Abort.\n");
    return 2;
  }

  int zr_port;

  if(!config_lookup_int(&cfg, "ZR_PORT", &zr_port))
  {
    fprintf(stderr, "no setting ZR_PORT in configuration file. Abort.\n");
    return 2;
  }
...
#define PKT_BUFSIZ 2000 
    handle = pcap_open_live(capture_interface, PKT_BUFSIZ, 1, 500, errbuf);
  ...
  char pcap_rule[60];
  if (snprintf (pcap_rule, sizeof pcap_rule, "src host %s and udp port %d", zr_addr, zr_port) >= (sizeof pcap_rule))
  {
      fprintf(stderr, "ZR_HOST or ZR_PORT parameter invalid\n");
      return 2;
  }


  if (pcap_compile(handle, &fp, pcap_rule, 0, 0) == -1)
  {
    fprintf(stderr, "Couldn't parse filter %s: %s\n", pcap_rule, pcap_geterr(handle));
    return 4;
  }
  ...
  while(1)
  {
    ...
    if (retval < 0)
    {
      printf( "select failed, stop (errno=%d)\n", errno);
      fflush(stdout);
      break;
    }
